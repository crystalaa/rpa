...
class LoginAndDirect:
    def __init__(self, base_pw: BasePw, base_url: str, debug_mode: bool = False):
        ...
        self.base_pw.load_selectors("湖北银企对账")
        logger.debug("湖北银企对账RPA初始化完成")
...
class OperateRobots:
    def __init__(self, base_robot: LoginAndDirect):
        ...
        logger.info("湖北银企对账初始化完成")

    def process_data(self, account_info: list, start_date: str):
        """提交请求"""
        try:
            logger.info("第四步：处理湖北银企对账", extra={'color': 'darkcyan'})
        ...
        with sync_playwright() as playwright:
            ...
            with BasePw(playwright, show_browser, record_video) as base_pw:
                ...
                if email_notif not in ['', '否', 'n', 'no', '不发送']:
                    send_email_notification(result=result, task_name="湖北银企对账")
                else:
                    logger.debug("通知邮件已关闭")

                logger.info("湖北银企对账已完成", extra={'color': 'green'})
                logger.debug(f"result: {result}")
                return result

    except RobotsException as e:
        logger.error(f"湖北银企对账执行出错: {e.message}",
                     exc_info=config.get_exc_info(),
                     stack_info=config.get_stack_info())
        send_email_alert(errmsg=e, base_pw=base_robot, task_name="湖北银企对账")
        raise
    except Exception as e:
        logger.error(f"湖北银企对账执行出错: {str(e)}",
                     exc_info=config.get_exc_info(),
                     stack_info=config.get_stack_info())
        send_email_alert(errmsg=e, base_pw=base_robot, task_name="湖北银企对账")
        raise
