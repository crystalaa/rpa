...
1、安徽电力资金协同审核
产业单位或直属单位与供电公司的电费业务
筛选“对方单位” 为 “国网安徽省电力有限公司本部” ，
摘要包含“社保、公积金、社会保险、个税、个人所得税、五险一金、企业年金、补充医疗保险”的记录并全部同意
'''
...
class PeiXunZhongXinRobots:
    def __init__(self, base_robot: ReatedPartyTrans):
        ...
        logger.info("安徽电力资金协同审核（社保公积金）RPA初始化完成")
    ...
    def process_data(self):
        """提交请求"""
        try:
            item_list = ['社保', '公积金', '社会保险', '个税', '个人所得税', '五险一金', '企业年金', '补充医疗保险']
            ...
            for item in item_list:
                logger.info(f"开始处理：“{item}”")
                counter = counter + 1
                if counter > 1:
                    self.base_pw.locate_and_click("资金协同审核-筛选条件", sleep=1000)
                else:
                    self.base_pw.locate_and_click("资金协同审核-筛选条件更多", sleep=1000)
                self.base_pw.locate_and_fill("资金协同审核-摘要", item)
                ...
                if response_info.is_done():
                    ...
                    try:
                        ...
                        if len(response_data) > 0:
                            ...
                            self.write_excel_json(data_map, response_data, sheet_name='社保公积金',
                    ...
                    if count_str != "共0条":
                        ...
                        try:
                            ...
                            logger.info(f"资金协同审核-{item}（有记录）-弹出框已打开")
                            confirm_button = self.base_pw.page.locator(
                                ".colorModal.modal.fade.in .modal-footer button[buttonindex='1'].btn-primary")
                            logger.debug(f"资金协同审核-{item}（有记录）-确认按钮：{confirm_button}")
                            confirm_button.click()
                        except Exception as e:
                            logger.warning(f"资金协同审核-{item}（有记录）-弹出框打开失败：{e}")
                            self.result_screenshot = self.base_pw.take_screenshot(f"1、资金协同审核-{item}（有记录）",
                                                                                  tag='运行日志')
                            raise RobotsException(f"资金协同审核-{item}（有记录）-弹出确认失败: {str(e)}", e)
                        self.base_pw.page.wait_for_timeout(3000)
                    else:
                        self.result_screenshot = self.base_pw.take_screenshot(f"1、资金协同审核-{item}（无记录）", tag='运行日志')
                        self.result_remark = f"没有“{item}”数据，不做任何处理"
                        logger.info(f'没有“{item}”数据，不做任何处理', extra={'color': 'blue'})
                logger.info(f"资金协同审核:“{item}”已处理完成")
