...
class GeRenKouChuRobots:
    def __init__(self, base_robot: ReatedPartyTrans):
        ...
        logger.info("安徽电力关联交易协同（水电费）RPA初始化完成")
    ...
    def process_data(self):
        ...
        try:
            ...
            logger.info(f"开始处理：“水电费”")
            # self.base_pw.page.pause()
            self.base_pw.locate_and_click("关联交易审核-列头-科目", sleep=1000)
            # self.base_pw.locate_and_click("关联交易审核-列头筛选按钮-内部往来单位", sleep=1000)
            self.base_pw.page.get_by_role("cell", name="科目", exact=True).get_by_role("cell").click()
            self.base_pw.page.wait_for_timeout(1000)
            self.base_pw.locate_and_fill("关联交易审核-列头查询弹窗-关键字输入框", "水电费", sleep=1000)
            ...
            if rows.count() > 1:
                ...
                self.result_screenshot = self.base_pw.take_screenshot(f"3、关联交易审核-水电费", tag="运行日志")
                ...
                self.result_screenshot = self.base_pw.take_screenshot(f"3、关联交易审核-水电费（无记录）", tag='运行日志')
                self.result_remark = "没有“水电费”数据，不做任何处理"
                logger.info(f'没有“水电费”数据，不做任何处理' , extra={'color': 'blue'})
            logger.debug(f"“水电费”处理完成")
            # self.base_pw.page.pause()
    ...
    def run(self, username: str, password: str):
        """运行完整的补采流程"""
        try:
            logger.debug("开始运行安徽电力关联交易(水电费)协同流程")
            ...
            self.base_pw.load_selectors("安徽电力关联交易协同-水电费")
            self.process_data()
            ...
            logger.info("安徽电力关联交易协同(水电费)流程执行完成")
            return result
...
def main(**kwargs):
    ...
    try:
        ...
        logger.info("开始执行安徽电力关联交易协同(水电费)")
        ...
        with sync_playwright() as playwright:
            ...
            with BasePw(playwright, show_browser, record_video) as base_pw:
                ...
                if email_notif not in ['','否', 'n', 'no', '不发送']:
                    send_email_notification(result=result, task_name="安徽电力关联交易审核（水电费）")
                else:
                    logger.debug("通知邮件已关闭")
                logger.info("安徽电力关联交易协同(水电费)已完成", extra={'color': 'green'})
                logger.debug(f"result: {result}")
                return result

    except RobotsException as e:
        logger.error(f"安徽电力关联交易协同(水电费)执行出错: {e.message}",
                     exc_info=config.get_exc_info(),
                     stack_info=config.get_stack_info())
        send_email_alert(errmsg=e, base_pw=base_robot, task_name="安徽电力关联交易审核（水电费）")
        raise
    except Exception as e:
        logger.error(f"安徽电力关联交易协同(水电费)执行出错: {str(e)}",
                     exc_info=config.get_exc_info(),
                     stack_info=config.get_stack_info())
        send_email_alert(errmsg=e, base_pw=base_robot, task_name="安徽电力关联交易审核（水电费）")
        raise
