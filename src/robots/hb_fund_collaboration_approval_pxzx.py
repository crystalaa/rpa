...
class ReatedPartyTrans:
    ...
    def navigate_to_trans_page(self):
        ...
        try:
            ...
            raise RobotsException(f"导航到资金协同审核页面失败: {str(e)}", e)
...
class PeiXunZhongXinRobots:
    def __init__(self, base_robot: ReatedPartyTrans):
        ...
        logger.info("安徽电力资金协同审核（培训中心）RPA初始化完成")
    ...
    def process_data(self):
        ...
        try:
            ...
            self.base_pw.locate_and_fill("资金协同审核-对方单位下拉框", "国网安徽省电力有限公司培训中心")
            self.base_pw.page.locator('div#select2-drop .select2-result-label:has-text("国网安徽省电力有限公司培训中心")').click()
            ...
            if response_info.is_done():
                ...
                try:
                    ...
                    if len(response_data) > 0:
                        ...
                        self.write_excel_json(data_map, response_data, sheet_name='培训中心',
                ...
                if count_str != "共0条":
                    ...
                    try:
                        ...
                        logger.info("资金协同审核-培训中心（有记录）-弹出框已打开")
                        confirm_button = self.base_pw.page.locator(".colorModal.modal.fade.in .modal-footer button[buttonindex='1'].btn-primary")
                        logger.debug(f"资金协同审核-培训中心（有记录）-确认按钮：{confirm_button}")
                        confirm_button.click()
                    except Exception as e:
                        logger.warning(f"资金协同审核-培训中心（有记录）-弹出框打开失败：{e}")
                        self.result_screenshot = self.base_pw.take_screenshot("1、资金协同审核-培训中心（有记录）", tag='运行日志')
                        raise RobotsException(f"资金协同审核-培训中心（有记录）-弹出确认失败: {str(e)}", e)
                else:
                    self.result_screenshot = self.base_pw.take_screenshot("1、资金协同审核-培训中心（无记录）", tag='运行日志')
    ...
    def run(self, username: str, password: str):
        ...
        try:
            ...
            # self.base_pw.load_selectors("安徽电力资金协同审核-培训中心")
            self.process_data()
            ...
            logger.debug("安徽电力资金协同审核（培训中心）协同流程执行完成")
            return result
...
def main(**kwargs):
    ...
    try:
        ...
        logger.info("开始执行安徽电力资金协同审核（培训中心）协同")
        base_robot = None
        ...
        with sync_playwright() as playwright:
            ...
            with BasePw(playwright, show_browser, record_video) as base_pw:
                ...
                if email_notif not in ['','否', 'n', 'no', '不发送']:
                    send_email_notification(result=result, task_name="安徽电力资金协同审核（培训中心）")
                else:
        ...
        logger.error(f"安徽电力资金协同审核（培训中心）协同执行出错: {e.message}",
                     exc_info=config.get_exc_info(),
                     stack_info=config.get_stack_info())
        send_email_alert(errmsg= e, base_pw = base_robot, task_name= "安徽电力资金协同审核（培训中心）")
        raise
    except Exception as e:
        logger.error(f"安徽电力资金协同审核（培训中心）协同执行出错: {str(e)}",
                     exc_info=config.get_exc_info(),
                     stack_info=config.get_stack_info())
        send_email_alert(errmsg= e, base_pw= base_robot, task_name= "安徽电力资金协同审核（培训中心）")
        raise
