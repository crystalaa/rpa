...
def generate_date_ranges(start_year_month: str, end_date: datetime = None) -> List[Tuple[str, str]]:
    if end_date is None:
...
class LoginAndDirect:
    def __init__(self, base_pw: BasePw, base_url: str, debug_mode: bool = False):
        ...
        self.base_pw.load_selectors("安徽银企对账")
        logger.debug("安徽银企对账RPA初始化完成")
    ...
    def navigate_to_trans_page(self):
        ...
        try:
            ...
            with self.base_pw.page.expect_popup() as page2:
                self.base_pw.locate_and_click("页面导航-银企对账", sleep=3000)
            self.base_pw.set_page(page2.value)
            logger.info("成功打开银企对账页面")
        except Exception as e:
            raise RobotsException(f"导航到银企对账页面失败: {str(e)}", e)
...
class OperateRobots:
    def __init__(self, base_robot: LoginAndDirect):
        ...
        logger.info("安徽银企对账初始化完成")

    def process_data(self, account_info: list, start_date: str):
        """提交请求"""
        try:
            logger.info("第四步：处理安徽银企对账", extra={'color': 'darkcyan'})
            self.base_pw.page.wait_for_load_state("load")
            date_range = generate_date_ranges(start_date)
            acc_element_css = self.base_pw.selectors.get("查询面板及按钮-对账账号输入框")
            for acc in account_info:
                for start_date, end_date in date_range:
                    self.base_pw.page.reload()
                    set_input_value(self, acc_element_css, acc['账号'])
                    self.base_pw.locate_and_fill("查询面板及按钮-对账期间", start_date[:7])
                    self.base_pw.locate_and_click("查询面板及按钮-查询按钮", sleep=3000)
                    self.base_pw.locate_and_click("查询面板及按钮-获取凭证数据按钮")
                    self.base_pw.locate_and_fill("查询面板及按钮-同步数据开始时间", start_date)
                    self.base_pw.locate_and_fill("查询面板及按钮-同步数据结束时间", end_date)
                    self.base_pw.locate_and_click("查询面板及按钮-同步数据弹窗确认按钮")
        ...
        with sync_playwright() as playwright:
            ...
            with BasePw(playwright, show_browser, record_video) as base_pw:
                ...
                if email_notif not in ['', '否', 'n', 'no', '不发送']:
                    send_email_notification(result=result, task_name="安徽银企对账")
                else:
                    logger.debug("通知邮件已关闭")

                logger.info("安徽银企对账已完成", extra={'color': 'green'})
                logger.debug(f"result: {result}")
                return result

    except RobotsException as e:
        logger.error(f"安徽银企对账执行出错: {e.message}",
                     exc_info=config.get_exc_info(),
                     stack_info=config.get_stack_info())
        send_email_alert(errmsg=e, base_pw=base_robot, task_name="安徽银企对账")
        raise
    except Exception as e:
        logger.error(f"安徽银企对账执行出错: {str(e)}",
                     exc_info=config.get_exc_info(),
                     stack_info=config.get_stack_info())
        send_email_alert(errmsg=e, base_pw=base_robot, task_name="安徽银企对账")
        raise
