...
'''
2、安徽电力关联交易协同-自动清账
筛选“分录摘要” 为 “自动清账” 的记录并全部同意
'''

class ZiDongQingZhangRobots:
    def __init__(self, base_robot: ReatedPartyTrans):
        ...
        logger.info("安徽电力关联交易协同（自动清账）RPA初始化完成")
    ...
    def process_data(self):
        ...
        try:
            ...
            self.base_pw.locate_and_fill("关联交易审核-列头查询弹窗-关键字输入框", "自动清账", sleep=1000)
            ...
            if rows.count() > 1:
                ...
                self.result_screenshot = self.base_pw.take_screenshot("2、关联交易审核-自动清账", tag="运行日志")
                ...
                self.result_screenshot = self.base_pw.take_screenshot("2、关联交易审核-自动清账（无记录）", tag='运行日志')
                self.result_remark = "没有“自动清账”数据，不做任何处理"
                logger.info(f'没有“自动清账”数据，不做任何处理' , extra={'color': 'blue'})
            # self.base_pw.page.pause()
    ...
    def run(self, username: str, password: str):
        """运行完整的补采流程"""
        try:
            logger.debug("开始运行安徽电力关联交易(自动清账)协同流程")
            ...
            logger.debug("安徽电力关联交易协同(自动清账)流程执行完成")
            return result
...
def main(**kwargs):
    ...
    try:
        ...
        logger.info("开始执行安徽电力关联交易协同(自动清账)")
        ...
        with sync_playwright() as playwright:
            ...
            with BasePw(playwright, show_browser, record_video) as base_pw:
                ...
                pxzx_robot = ZiDongQingZhangRobots(base_robot)
                result = pxzx_robot.run(username, password)

                if email_notif not in ['','否', 'n', 'no', '不发送']:
                    send_email_notification(result=result, task_name="安徽电力关联交易审核（自动清账）")
                else:
                    logger.debug("通知邮件已关闭")
                logger.info("安徽电力关联交易协同(自动清账)已完成", extra={'color': 'green'})
                logger.debug(f"result: {result}")
                return result

    except RobotsException as e:
        logger.error(f"安徽电力关联交易协同(自动清账)执行出错: {e.message}",
                     exc_info=config.get_exc_info(),
                     stack_info=config.get_stack_info())
        send_email_alert(errmsg=e, base_pw=base_robot, task_name="安徽电力关联交易审核（自动清账）")
        raise
    except Exception as e:
        logger.error(f"安徽电力关联交易协同(自动清账)执行出错: {str(e)}",
                     exc_info=config.get_exc_info(),
                     stack_info=config.get_stack_info())
        send_email_alert(errmsg=e, base_pw=base_robot, task_name="安徽电力关联交易审核（自动清账）")
        raise
