...
class LoginAndDirect:
    def __init__(self, base_pw: BasePw, base_url: str, debug_mode: bool = False):
        ...
        self.base_pw.load_selectors("安徽多维成本归集分摊")
        logger.debug("安徽多维成本归集分摊RPA初始化完成")
    ...
    def navigate_to_trans_page(self):
        """导航到”多维成本分摊“页面"""
        try:
            logger.info("第二步：导航到”多维成本分摊“页面", extra={'color': 'darkcyan'})
            ...
            with self.base_pw.page.expect_popup() as page1:
                self.base_pw.locate_and_click("页面导航-多维成本分摊", sleep=1000)
                logger.debug(f'多维成本分摊 popped up , {page1.value}')

            self.base_pw.set_page(page1.value)
            self.base_pw.page.wait_for_load_state("networkidle")
            self.base_pw.page.wait_for_timeout(3000)

            logger.info("成功打开”多维成本分摊“页面")
        except Exception as e:
            raise RobotsException(f"导航到”多维成本分摊“页面失败: {str(e)}", e)

    def navigate_to_target_page(self):
        """导航到”多维成本归集“页面"""
        try:
            logger.info("第三步：导航到”多维成本归集“页面", extra={'color': 'darkcyan'})
            with self.base_pw.page.expect_popup() as page2:
                self.base_pw.locate_and_click("页面导航-左侧菜单-展开", sleep=3000)
                self.base_pw.locate_and_fill("页面导航-左侧菜单-搜索输入框", "多维成本归集")
                self.base_pw.locate_and_click("页面导航-左侧菜单-选择多维成本归集菜单", sleep=1000)
            self.base_pw.set_page(page2.value)
            self.base_pw.page.wait_for_load_state("networkidle")
            self.base_pw.page.wait_for_timeout(3000)

            logger.info("成功打开”多维成本归集“页面")
        except Exception as e:
            raise RobotsException(f"导航到”多维成本归集“页面失败: {str(e)}", e)

    def navigate_to_target_page1(self):
        """导航到”多维成本分摊“页面"""
        try:
            logger.info("第五步：导航到”多维成本分摊“页面", extra={'color': 'darkcyan'})
            with self.base_pw.page.expect_popup() as page2:
                self.base_pw.locate_and_fill("页面导航-左侧菜单-搜索输入框", "多维成本分摊")
                self.base_pw.locate_and_click("页面导航-左侧菜单-选择多维成本分摊菜单", sleep=1000)
            self.base_pw.set_page(page2.value)
            self.base_pw.page.wait_for_load_state("networkidle")
            self.base_pw.page.wait_for_timeout(3000)
...
class OperateQueryAndExportRobots:
    def __init__(self, base_robot: LoginAndDirect):
        ...
        logger.info("安徽多维成本归集分摊RPA初始化完成")

    def process_data(self):
        """提交请求"""
        try:
            logger.info("第四步：点击取数按钮，打开选择单位弹窗；右键选择：全选->所有底层节点；点击确定进行取数", extra={'color': 'darkcyan'})

            # 1. 定位取数按钮并点击
            self.base_pw.locate_and_click("查询面板-取数按钮", sleep=1000)

            # 2. 选择单位弹窗
            self.choose_unit_model()

            # 3. 等待操作结果状态
            self.wait_for_opera_state()
    ...
    def process_data1(self):
        """提交请求"""
        try:
            logger.info("第六步：点击全量分摊按钮，打开选择单位弹窗；右键选择：全选->所有底层节点；点击确定进行全量分摊", extra={'color': 'darkcyan'})

            # 定位全量分摊按钮并点击
            self.base_pw.locate_and_click("查询面板-全量分摊按钮", sleep=1000)
    ...
    def choose_unit_model(self):
        """选择单位弹窗"""
        # 1. 定位选择单位弹窗中“右键点击省公司节点”并右键点击
        selectors = self.base_pw.selectors.get("查询面板-选择单位弹窗-右键点击省公司节点")
        locator = self.base_pw.page.locator(selectors)
        locator.click(button="right")

        # 2. 定位选择单位弹窗中“全选”并点击
        self.base_pw.locate_and_click("查询面板-选择单位弹窗-全选节点", sleep=500)

        # 3. 定位选择单位弹窗中“所有底层节点”并点击选中
        self.base_pw.locate_and_click("查询面板-选择单位弹窗-所有底层节点", sleep=3000)

        # 4. 定位选择单位弹窗中“确定按钮”并点击
        self.base_pw.locate_and_click("查询面板-选择单位弹窗-确定按钮", sleep=5000)

    def wait_for_opera_state(self):
        """
        等待操作结果状态
        :raises RobotsException: 关键步骤超时/定位失败时抛出异常（含详细日志）
        """
        # 定义核心配置（便于维护）
        max_wait_time = 1800  # 等待导出记录最大超时（秒）
        check_interval = 2  # 轮询检查间隔（秒）
        logger.info(f"开始等待操作结果状态...")

        opera_flag = False
        start_time = time.time()
        logger.info(f"开始等待操作结果状态记录生成（最大等待{max_wait_time}秒）")
        while time.time() - start_time < max_wait_time:
            try:
                selectors = self.base_pw.selectors.get("查询面板-弹窗-完成标识")
                row_locator = self.base_pw.page.locator(selectors)

                if row_locator.count() > 0:
                    logger.info(f"匹配到操作结果状态记录，结果为：{row_locator.first.inner_text()}")
                    opera_flag = True
                    break

                selectors = self.base_pw.selectors.get("查询面板-弹窗-失败标识")
                row_locator = self.base_pw.page.locator(selectors)
                ...
                logger.debug(f"本轮查找等待操作结果状态记录异常（忽略，继续轮询）：{str(e)}")

            # 轮询间隔
            time.sleep(check_interval)
            elapsed = int(time.time() - start_time)
            logger.debug(f"操作结果状态记录等待中，已耗时{elapsed}秒（剩余{max_wait_time - elapsed}秒）")

        # 等待操作结果状态记录超时判断
        if not opera_flag:
            error_msg = f"导等待操作结果状态记录生成超时（{max_wait_time}秒），未匹配到目标记录"
            logger.error(error_msg, exc_info=True)
            raise RobotsException(error_msg)
        else:
            logger.info(f"等待操作结果状态记录定位成功")

        # 关闭弹窗
        self.base_pw.locate_and_click("查询面板-弹窗-关闭按钮", sleep=3000)
        logger.info(f"关闭弹窗成功")

        # 等待分摊完成提示弹窗
        self.base_pw.page.wait_for_selector("div.modal:has-text('分摊完成提示')", timeout=5000)
        is_modal_visible = self.base_pw.page.locator("div.modal:has-text('分摊完成提示')").is_visible()
        if is_modal_visible:
            # 获取数据条数（兼容定位失败场景）
            try:
                data_count_locator = self.base_pw.page.locator("td#common_td:has-text('共0条')")
                data_count = data_count_locator.count()
                logger.info(f"数据条数定位元素数量：{data_count}")
            except Exception as e:
                logger.error(f"获取数据条数定位失败，详情：{str(e)}", exc_info=True)
                data_count = 0  # 定位失败默认按有数据处理

            # 执行导出分摊失败数据详情并发送到指定邮箱操作
            if data_count == 0:
                self.export_and_send(is_modal_visible)


    def export_and_send(self, is_export_and_send: bool = False):
        """
        导出分摊失败数据详情并发送到指定邮箱
        :raises RobotsException: 关键步骤超时/定位失败时抛出异常（含详细日志）
        """

        if is_export_and_send:
            logger.info(f"开始执行导出下载操作...")
            try:
                # 定位导出按钮
                export_button = self.base_pw.page.locator("div.modal-footer button.btn-default:has-text('导出')")
                # 等待导出下载事件
                with self.base_pw.page.expect_download(timeout=30000) as download_info:
                    export_button.click()
                    logger.info(f"点击导出按钮，等待文件下载...")
                ...
                new_file_name = f"{file_name_parts[0]}{file_name_parts[1]}".replace('\\', '_')
                save_dir = 'data'
                ...
                logger.info(f"文件导出下载完成，保存路径：{save_path}")

            except Exception as e:
                logger.error(f"文件导出下载失败，详情：{str(e)}", exc_info=True)
                raise RobotsException(f"文件导出下载失败") from e

            # 关闭分摊完成提示弹窗
            self.base_pw.page.wait_for_timeout(3000)
            self.base_pw.locate_and_click("div.modal-footer button.btn-primary:has-text('确定')", sleep=1000)
            logger.info(f"关闭分摊完成提示弹窗成功")

            # 发送分摊失败信息文件
            try:
                send_rpa_success_notification(
                    task_name="安徽多维成本归集分摊",
                    result_message=str("分摊完成提示"),
                    from_addr="xtgscwgk@ah.sgcc.com.cn",
                    to_addr="xtgscwgk@ah.sgcc.com.cn",
                    attachments=[save_path],
                    additional_info="详细信息请参见附件"
                )
    ...
    def run(self, username: str, password: str):
        ...
        try:
            ...
            logger.debug("安徽多维成本归集分摊-查询导出流程执行完成")
            return result
        ...
        with sync_playwright() as playwright:
            ...
            with BasePw(playwright, show_browser, record_video) as base_pw:
                ...
                if email_notif not in ['','否', 'n', 'no', '不发送']:
                    send_email_notification(result=result, task_name="安徽多维成本归集分摊")
                else:
                    logger.debug("通知邮件已关闭")

                logger.info("安徽多维成本归集分摊已完成", extra={'color': 'green'})
                logger.debug(f"result: {result}")
                return result

    except RobotsException as e:
        logger.error(f"安徽多维成本归集分摊执行出错: {e.message}",
                     exc_info=config.get_exc_info(),
                     stack_info=config.get_stack_info())
        send_email_alert(errmsg= e, base_pw = base_robot, task_name= "安徽多维成本归集分摊")
        raise
    except Exception as e:
        logger.error(f"安徽多维成本归集分摊执行出错: {str(e)}",
                     exc_info=config.get_exc_info(),
                     stack_info=config.get_stack_info())
        send_email_alert(errmsg= e, base_pw= base_robot, task_name= "安徽多维成本归集分摊")
        raise
